"""Find ONLY 2025 season mock drafts (not real league drafts)"""
...
def find_2025_mock_drafts_only():
    """Find only 2025 season mock drafts, excluding all real league drafts."""
    load_dotenv()
    ...
    print(f"=== Finding 2025 MOCK DRAFTS ONLY for {username} ===\n")
    ...
    try:
        ...
        try:
            ...
            if not drafts:
                ...
                print(f"\n❌ No mock drafts found for {username} in 2025")
                print("\nThis could mean:")
                print("1. You haven't run any mock drafts in 2025 yet")
        ...
        mock_drafts.sort(key=lambda x: x['created'], reverse=True)
        
        print(f"\n=== Found {len(mock_drafts)} Mock Drafts in 2025 ===")
        print("-" * 90)
        ...
        completed_mocks = [d for d in mock_drafts if d['status'] == 'complete']
        ...
        if selection == 'all':
            ...
            try:
                ...
                selected_drafts = [mock_drafts[i] for i in indices if 0 <= i < len(mock_drafts)]
        ...
        print("Enter players that will be keepers in your upcoming 2025 draft (one per line, empty line to finish):")
        ...
        for draft in selected_drafts:
            try:
                print(f"\nImporting 2025 mock draft {draft['draft_id']} ({draft['created'].strftime('%Y-%m-%d')})...")
                mock_draft = importer.import_draft_by_id(draft['draft_id'], keepers)
                
                # Add mock draft info to notes
                mock_draft.notes = f"2025 Mock Draft - {mock_draft.notes}"
        ...
        if imported_count > 0:
            print(f"\n🎉 Successfully imported {imported_count} mock drafts from 2025!")
            ...
            if adp_data:
                print(f"\n=== 2025 Mock Draft ADP Analysis (Top 25) ===")
            ...
            # Export option
            export = input(f"\nExport 2025 mock draft ADP to CSV? (y/n): ").strip().lower()
            if export == 'y':
                filename = importer.tracker.export_adp_to_csv("2025_mock_draft_adp.csv")
